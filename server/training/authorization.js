...
import { supabaseTrainingUserById } from '../supabase/admin.js'
...
export async function currentTrainingUser(req) {
  ...
  const user = await supabaseTrainingUserById(session.sub) ?? await trainingUserRepository.findById(session.sub)
